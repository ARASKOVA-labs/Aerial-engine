...
use crate::scene::{ElementKind, Scene, SceneElement};
...
impl AerialCanvas {
    ...
    pub fn set_tool_freedraw(&mut self) {
        self.active_tool = ActiveTool::FreeDraw;
    }
    pub fn set_tool_rectangle(&mut self) {
        self.active_tool = ActiveTool::Rectangle;
    }
    pub fn set_tool_ellipse(&mut self) {
        self.active_tool = ActiveTool::Ellipse;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_line(&mut self) {
        self.active_tool = ActiveTool::Line;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_arrow(&mut self) {
    ...
    pub fn set_tool_hand(&mut self) {
        self.active_tool = ActiveTool::Hand;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_highlighter(&mut self) {
        self.active_tool = ActiveTool::Highlighter;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_fountain_pen(&mut self) {
        self.active_tool = ActiveTool::FountainPen;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_text(&mut self) {
        self.active_tool = ActiveTool::Text;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_eraser(&mut self) {
        self.active_tool = ActiveTool::Eraser;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_magic_pen(&mut self) {
        self.active_tool = ActiveTool::MagicPen;
        self.selected_ids.clear();
        self.render();
    }
    pub fn set_tool_laser_pen(&mut self) {
        self.active_tool = ActiveTool::LaserPen;
        self.selected_ids.clear();
        self.render();
    }
    ...
    pub fn tick_animations(&mut self) -> bool {
        ...
        if !ids_to_remove.is_empty() {
            ...
            self.scene
                .elements
                .retain(|el| !ids_to_remove.contains(&el.id));
            self.is_dirty = true;
            needs_render = true;
        }
        ...
        if let Ok(scene) = serde_json::from_str::<Scene>(json) {
            ...
            for el in &self.scene.elements {
                if let Ok(json_str) = serde_json::to_string(el) {
                    self.elements_map
                        .insert(&mut txn, el.id.to_string(), json_str);
                }
            }
            self.is_dirty = true;
            self.render();
    ...
    pub fn add_text(&mut self, text: &str, x: f64, y: f64, size: f64, font_family: Option<String>) {
        let mut el = SceneElement::new_text(
            self.scene.next_id(),
            x,
            y,
            text.to_string(),
            ...
            ActiveTool::Select => {
                ...
                for &sel_id in &self.selected_ids {
                    if let Some(el) = self.scene.get_element(sel_id) {
                        ...
                        let w = if el.kind == ElementKind::Text {
                            el.width.abs().max(60.0)
                        } else {
                            el.width
                        };
                        let h = if el.kind == ElementKind::Text {
                            el.height.abs().max(30.0)
                        } else {
                            el.height
                        };
                        if (x - (el.x + w)).abs() < handle_size
                            && (y - (el.y + h)).abs() < handle_size
                        {
                            clicked_handle = true;
                ...
                if let Some((sx, sy)) = self.selection_box_start {
                    ...
                    for start_el in &self.drag_start_elements {
                        if let Some(el) = self.scene.get_element_mut(start_el.id) {
                            if self.is_resizing {
                                let w = if start_el.kind == ElementKind::Text {
                                    start_el.width.abs().max(60.0)
                                } else {
                                    start_el.width
                                };
                                let h = if start_el.kind == ElementKind::Text {
                                    start_el.height.abs().max(30.0)
                                } else {
                                    start_el.height
                                };
                                ...
                                if start_el.kind == ElementKind::FreeDraw {
                                    el.points = start_el
                                        .points
                                        .iter()
                                        .map(|pt| [pt[0] * scale_x, pt[1] * scale_y])
                                        .collect();
    ...
    pub fn on_mouse_up(&mut self, event: &MouseEvent) {
        ...
        match self.active_tool {
            ...
            ActiveTool::Line => {
                let mut el = SceneElement::new_rect(
                    self.scene.next_id(),
                    self.start_x,
                    self.start_y,
                    x - self.start_x,
    ...
    fn event_to_canvas(&self, event: &MouseEvent) -> (f64, f64) {
        ...
        (
            (raw_x - self.pan_x) / self.zoom,
            (raw_y - self.pan_y) / self.zoom,
        )
    }

    fn draw_grid(&self, viewport_w: f64, viewport_h: f64) {
        if self.grid_type == "blank" {
            return;
        }
        ...
        if self.grid_type == "lines" {
            ...
            while gx <= world_end_x {
                ...
                while gy <= world_end_y {
                    self.ctx.begin_path();
                    self.ctx
                        .arc(gx, gy, 0.5, 0.0, std::f64::consts::TAU)
                        .unwrap();
                    self.ctx.fill();
                    gy += GRID_SPACING;
                }
    ...
    fn draw_element(&self, el: &SceneElement) {
        self.ctx
            .set_stroke_style_str(&self.get_adaptive_color(&el.stroke));
        self.ctx
            .set_fill_style_str(&self.get_adaptive_color(&el.fill));
        ...
        match el.kind {
            ElementKind::Rectangle => {
                ...
                if el.is_rough {
                    ...
                    for _ in 0..2 {
                        // Double stroke for scribble effect
                        let max_offset = 3.0;
                        self.ctx.move_to(
                            el.x + next_rand() * max_offset,
                            el.y + next_rand() * max_offset,
                        );
                        self.ctx.line_to(
                            el.x + el.width + next_rand() * max_offset,
                            el.y + next_rand() * max_offset,
                        );
                        ...
                        self.ctx.line_to(
                            el.x + el.width + next_rand() * max_offset,
                            el.y + el.height + next_rand() * max_offset,
                        );
            ...
            ElementKind::FreeDraw => {
                ...
                let draw_path = |ctx: &CanvasRenderingContext2d| {
                    ...
                    if len < 3 {
                        ...
                        ctx.quadratic_curve_to(
                            el.x + el.points[len - 2][0],
                            el.y + el.points[len - 2][1],
                            el.x + el.points[len - 1][0],
                            el.y + el.points[len - 1][1],
                        );
                    }
                };
                            ...
                            if let Ok(raster_canvas) = canvas_el.dyn_into::<HtmlCanvasElement>() {
                                ...
                                if nw > 0 && nh > 0 {
                                    ...
                                    if let Ok(Some(rctx_obj)) = raster_canvas.get_context("2d") {
                                        if let Ok(rctx) =
                                            rctx_obj.dyn_into::<CanvasRenderingContext2d>()
                                        {
                                            let _ = rctx
            ...
            ElementKind::Text => {
                if let Some(ref text) = el.text {
                    ...
                    self.ctx
                        .set_fill_style_str(&self.get_adaptive_color(&el.stroke));
    ...
    fn draw_selection_box(&self, el: &SceneElement) {
        ...
        self.ctx.fill_rect(
            bx + bw - handle_size / 2.0,
            by + bh - handle_size / 2.0,
            handle_size,
            handle_size,
        );
    ...
    fn draw_live_freedraw(&self) {
        ...
        self.ctx.save();
        self.ctx
            .set_stroke_style_str(&self.get_adaptive_color(&self.stroke_color));
        ...
        let draw_path = |ctx: &CanvasRenderingContext2d| {
            ...
            let sy = self.start_y;
            ctx.move_to(
                sx + self.current_points[0][0],
                sy + self.current_points[0][1],
            );
            ...
            if len < 3 {
                ...
                for i in 1..(len - 2) {
                    ...
                    ctx.quadratic_curve_to(
                        sx + self.current_points[i][0],
                        sy + self.current_points[i][1],
                        sx + xc,
                        sy + yc,
                    );
                }
                ctx.quadratic_curve_to(
                    sx + self.current_points[len - 2][0],
                    sy + self.current_points[len - 2][1],
                    sx + self.current_points[len - 1][0],
                    sy + self.current_points[len - 1][1],
                );
            }
        };
    ...
    fn draw_live_ellipse(&self, x: f64, y: f64) {
        ...
        self.ctx.begin_path();
        self.ctx
            .ellipse(cx, cy, rw / 2.0, rh / 2.0, 0.0, 0.0, std::f64::consts::TAU)
            .unwrap();
        self.ctx.stroke();
        self.ctx.set_line_dash(&js_sys::Array::new()).unwrap();
        self.ctx.restore();
            ...
            if let Some(el) = self.scene.get_element_mut(id) {
                if el.kind == ElementKind::Text {
                    ...
                    if let Ok(json_str) = serde_json::to_string(&clone) {
                        self.elements_map
                            .insert(&mut txn, id.to_string().as_str(), json_str);
                    }
